...
async def generate_og_image(game: str) -> Response:
    ...
    if not HAS_PIL:
        ...
        raise HTTPException(status_code=501, detail="Tính năng sinh ảnh chưa được kích hoạt trên server")
    ...
    try:
        ...
        return Response(content=img_byte_arr, media_type="image/png", headers={
            "Cache-Control": "public, max-age=86400" # Cache 1 ngày để chống nghẽn server
        })
    except Exception as e:
        logger.error("Lỗi sinh OG Image: %s", e)
        raise HTTPException(status_code=500, detail="Lỗi tạo ảnh")
