...
def _adapter(ctx: dict[str, Any]) -> SteamCatalogAdapter:
    ...
    return SteamCatalogAdapter(
        AdapterConfig(limiter=RedisTokenBucket(clients.redis, "steam_appdetails", DETAILS_RATE_LIMIT)),
        clients.http,
...
async def sync_steam_prices(ctx: dict[str, Any]) -> dict[str, int]:
    ...
    # 1. Tìm 50 game cần check (mock hot/warm/cold bằng cách ưu tiên game chưa check hoặc check lâu nhất)
    ...
    ).sort([("price_checked_at", 1)]).limit(BATCH_SIZE)
    ...
    logger.info(
        ...
        extra={"batch": len(appids), "updated": res["updated"], "history": res["history_added"], "locked": len(locked_game_ids)}
    )
    return {
        "batch": len(appids),
        "updated": res["updated"],
