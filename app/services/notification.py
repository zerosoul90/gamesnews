...
async def send_push_notification(user_id: PyObjectId, title: str, body: str, data: dict[str, Any]) -> None:
    """Mock việc gửi FCM tới điện thoại người dùng."""
    logger.info("FCM MOCK: Đẩy push tức thì tới %s: [%s] %s", user_id, title, body, extra={"fcm_data": data})
...
async def process_notification(
    ...
    if payload.type == "price_alert" and not channels.get("price_alert", True):
        logger.info("Spam Prevented: User tắt kênh price_alert", extra={"user_id": str(payload.user_id)})
        return
    ...
    in_quiet = is_in_quiet_hours(now, quiet_hours.get("from", "22:00"), quiet_hours.get("to", "07:00"))
    ...
    if (is_immediate_type and not in_quiet) or force_immediate:
        ...
        logger.info("Notification kẹt queue (do Quiet Hours hoặc là Digest type)", extra={"user_id": str(payload.user_id)})
